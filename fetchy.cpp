...
namespace fetchy
{
    ...
    auto gpu_name = [](const char *query, std::string *name)
    {
      ...
      if (!result.empty())
      {
        ...
        if (colon_pos != std::string::npos)
          result = result.erase(0, colon_pos + 2);

        int paren_pos = result.find_last_of('(');
        if (paren_pos != std::string::npos)
          result = result.erase(paren_pos, result.length());
      }
  ...
  std::string Fetchy::GetDiskCapacity(bool home_partition_exists)
  {
    ...
    if (home_partition_exists)
      capacity += space_info_home.capacity;
    capacity /= (1024 * 1024 * 1024);

    unsigned long long int free = space_info_root.free;
    if (home_partition_exists)
      free += space_info_home.free;
    free /= (1024 * 1024 * 1024);
